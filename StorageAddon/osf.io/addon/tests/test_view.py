...
from addons.myminio.tests.utils import MyMinIOAddonTestCase
from addons.myminio.utils import validate_bucket_name
import addons.myminio.settings as myminio_settings
from website.util import api_url_for
from admin.rdm_addons.utils import get_rdm_addon_option

pytestmark = pytest.mark.django_db

class TestMyMinIOViews(MyMinIOAddonTestCase, OAuthAddonConfigViewsTestCaseMixin, OsfTestCase):
    def setUp(self):
        self.mock_can_list = mock.patch('addons.myminio.views.utils.can_list')
        self.mock_can_list.return_value = True
        self.mock_can_list.start()
        self.mock_uid = mock.patch('addons.myminio.views.utils.get_user_info')
        self.mock_uid.return_value = {'id': '1234567890', 'display_name': 'myminio.user'}
        self.mock_uid.start()
        self.mock_exists = mock.patch('addons.myminio.views.utils.bucket_exists')
        self.mock_exists.return_value = True
        self.mock_exists.start()
        super(TestMyMinIOViews, self).setUp()

    def tearDown(self):
        self.mock_can_list.stop()
        self.mock_uid.stop()
        self.mock_exists.stop()
        super(TestMyMinIOViews, self).tearDown()

    def test_myminio_settings_input_empty_keys(self):
        url = self.project.api_url_for('myminio_add_user_account')
    ...
    def test_myminio_settings_input_empty_access_key(self):
        url = self.project.api_url_for('myminio_add_user_account')
        rv = self.app.post_json(url, {
            'access_key': '',
    ...
    def test_myminio_settings_input_empty_secret_key(self):
        url = self.project.api_url_for('myminio_add_user_account')
        rv = self.app.post_json(url, {
    ...
    def test_myminio_settings_rdm_addons_denied(self):
    ...
    def test_myminio_set_bucket_no_settings(self):
        user = AuthUserFactory()
        self.project.add_contributor(user, save=True)
        url = self.project.api_url_for('myminio_set_config')
        res = self.app.put_json(
            url, {'myminio_bucket': 'hammertofall'}, auth=user.auth,
            expect_errors=True
        )
        assert_equal(res.status_code, http_status.HTTP_400_BAD_REQUEST)

    def test_myminio_set_bucket_no_auth(self):

        user = AuthUserFactory()
        user.add_addon('myminio')
        self.project.add_contributor(user, save=True)
        url = self.project.api_url_for('myminio_set_config')
        res = self.app.put_json(
            url, {'myminio_bucket': 'hammertofall'}, auth=user.auth,
    ...
    def test_myminio_set_bucket_registered(self):
        registration = self.project.register_node(
            get_default_metaschema(), Auth(self.user),
            DraftRegistrationFactory(branched_from=self.project), ''
        )

        url = registration.api_url_for('myminio_set_config')
        res = self.app.put_json(
            url, {'myminio_bucket': 'hammertofall'}, auth=self.user.auth,
            expect_errors=True,
        )

        assert_equal(res.status_code, http_status.HTTP_400_BAD_REQUEST)

    @mock.patch('addons.myminio.views.utils.can_list', return_value=False)
    def test_user_settings_cant_list(self, mock_can_list):
        url = api_url_for('myminio_add_user_account')
        rv = self.app.post_json(url, {
            'access_key': 'aldkjf',
            'secret_key': 'las'
        }, auth=self.user.auth, expect_errors=True)
    ...
    def test_myminio_remove_node_settings_owner(self):
        url = self.node_settings.owner.api_url_for('myminio_deauthorize_node')
    ...
    def test_myminio_remove_node_settings_unauthorized(self):
        url = self.node_settings.owner.api_url_for('myminio_deauthorize_node')
        ret = self.app.delete(url, auth=None, expect_errors=True)

        assert_equal(ret.status_code, 401)

    def test_myminio_get_node_settings_owner(self):
        self.node_settings.set_auth(self.external_account, self.user)
        self.node_settings.folder_id = 'bucket'
        self.node_settings.save()
        url = self.node_settings.owner.api_url_for('myminio_get_config')
    ...
    def test_myminio_get_node_settings_unauthorized(self):
        url = self.node_settings.owner.api_url_for('myminio_get_config')
    ...
    @mock.patch('addons.myminio.models.get_bucket_names')
    def test_folder_list(self, mock_names):
        mock_names.return_value = ['bucket1', 'bucket2']
        super(TestMyMinIOViews, self).test_folder_list()
...
class TestCreateBucket(MyMinIOAddonTestCase, OsfTestCase):
    ...
    def setUp(self):
        ...
        self.project.add_addon('myminio', auth=self.consolidated_auth)
        self.project.creator.add_addon('myminio')

        self.user_settings = self.user.get_addon('myminio')
        self.user_settings.access_key = 'We-Will-Rock-You'
        self.user_settings.secret_key = 'Idontknowanyqueensongs'
        self.user_settings.save()

        self.node_settings = self.project.get_addon('myminio')
        self.node_settings.bucket = 'Sheer-Heart-Attack'
        self.node_settings.user_settings = self.project.creator.get_addon('myminio')
    ...
    @mock.patch('addons.myminio.views.utils.create_bucket')
    @mock.patch('addons.myminio.views.utils.get_bucket_names')
    def test_create_bucket_pass(self, mock_names, mock_make):
        ...
        url = self.project.api_url_for('myminio_create_bucket')
    ...
    def test_create_bucket_fail(self, mock_make):
        ...
        url = '/api/v1/project/{0}/myminio/newbucket/'.format(self.project._id)
