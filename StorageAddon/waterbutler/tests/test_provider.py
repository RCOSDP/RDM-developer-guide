...
from waterbutler.providers.myminio import MyMinIOProvider
...
def provider(auth, credentials, settings):
    return MyMinIOProvider(auth, credentials, settings)
...
class TestProviderConstruction:
    ...
    def test_https(self, auth, credentials, settings):
        provider = MyMinIOProvider(auth, {'host': 'securehost',
                                           'access_key': 'a',
        ...
        provider = MyMinIOProvider(auth, {'host': 'securehost:443',
                                           'access_key': 'a',
                                           'secret_key': 's'}, settings)
        assert provider.connection.is_secure
        assert provider.connection.host == 'securehost'
    ...
    def test_http(self, auth, credentials, settings):
        provider = MyMinIOProvider(auth, {'host': 'normalhost:80',
                                           'access_key': 'a',
                                           'secret_key': 's'}, settings)
        ...
        provider = MyMinIOProvider(auth, {'host': 'normalhost:8080',
                                           'access_key': 'a',
                                           'secret_key': 's'}, settings)
        assert not provider.connection.is_secure
        assert provider.connection.host == 'normalhost'
