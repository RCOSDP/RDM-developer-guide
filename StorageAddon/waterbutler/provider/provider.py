...
from waterbutler.providers.myminio import settings
from waterbutler.providers.myminio.metadata import MyMinIORevision
from waterbutler.providers.myminio.metadata import MyMinIOFileMetadata
from waterbutler.providers.myminio.metadata import MyMinIOFolderMetadata
from waterbutler.providers.myminio.metadata import MyMinIOFolderKeyMetadata
from waterbutler.providers.myminio.metadata import MyMinIOFileMetadataHeaders
    ...
    * A GET prefix query against a non-existent path returns 200
    ...
    def __init__(self, auth, credentials, settings):
        ...
        .. note::
            ...
            Neither `MyMinIOConnection#__init__` nor `MyMinIOConnection#get_bucket`
            sends a request.
    ...
    async def intra_copy(self, dest_provider, source_path, dest_path):
        """Copy key from one My MinIO bucket to another. The credentials specified in
    ...
    async def upload(self, stream, path, conflict='replace', **kwargs):
        """Uploads the given stream to My MinIO

        :param waterbutler.core.streams.RequestWrapper stream: The stream to put to My MinIO
    ...
    async def create_folder(self, path, folder_precheck=True, **kwargs):
        ...
        async with self.request(
            ...
            return MyMinIOFolderMetadata({'Prefix': path.path})
    ...
    async def _metadata_file(self, path, revision=None):
        ...
        await resp.release()
        return MyMinIOFileMetadataHeaders(path.path, resp.headers)
    ...
    async def _metadata_folder(self, path):
        ...
        for content in contents:
            ...
            if content['Key'].endswith('/'):
                items.append(MyMinIOFolderKeyMetadata(content))
            else:
                items.append(MyMinIOFileMetadata(content))
